...
var obj = {
  ...
  gameStart: function() {
    ...
    document.onkeydown = function(e) { //自动获得事件对象
      ...
      getEleById("score").innerHTML = obj.score; //更新分数
    }
  ...
  init: function() {
    ...
    if (obj.createEle == 1) {
      obj.createEle = 0;
      getEleById("gridPanel").innerHTML = ""; //清空原有的元素
      getEleById("gridPanel").appendChild(obj.eleFragment); //添加元素
    }
    obj.score = 0;
    getEleById("score").innerHTML = obj.score;
    getEleById("game").style.display = "none";
    getEleById("gameover").style.display = "none";
  ...
  updateView: function() {
    ...
    for (r = 0; r < obj.ROW; r++) {
      for (c = 0; c < obj.CELL; c++) {
        if (arr[r][c] == 0) { //值为0的不显示
          getEleById("c" + r + c).innerHTML = ""; //0不显示
          getEleById("c" + r + c).className = "cell" //清除样式
        } else {
          getEleById("c" + r + c).innerHTML = arr[r][c];
          getEleById("c" + r + c).className = "cell n" + arr[r][c]; //添加不同数字的颜色
    ...
    if (win == 1) { //通关
      getEleById("game").style.display = "block";
      getEleById("gameover").style.display = "block";
      getEleById("score_over").innerHTML = "You win!<br>Score:" + obj.score;
    }
    if (obj.isGameOver()) { //游戏失败
      getEleById("game").style.display = "block";
      getEleById("gameover").style.display = "block";
      getEleById("score_over").innerHTML = "GAME OVER!<br>Score:" + obj.score;
...
//   var modelValue = parseInt(getEleById("model").value);
//   if (isNaN(modelValue)) {
//     modelValue = 4; //默认是4*4
//   }
